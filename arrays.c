...
double* full_elements(double* ptr_array, int size) {
    if (ptr_array == NULL || size <= 0) return NULL;   
...
int put_elements(double* ptr_array, int n) {
    if (ptr_array == NULL || n <= 0) return 0;   
    for (int i = 0; i < n; i++) {
...
double* calc_elements(double* ptr_array, int n) {
    if (ptr_array == NULL || n <= 0) return NULL;   
    for (int i = 0; i < n; i++) {
        if (ptr_array[i] < 0.0) {
            ptr_array[i] = 0.0;
        }
...
double sum_elements(double* ptr_array, int begin, int end, int n) {
    if (ptr_array == NULL || n <= 0) return 0.0;
    if (begin < 0 || end >= n || begin > end) return 0.0;   
    double sum = 0.0;
...
int find_element(double* ptr_array, int n, double element) {
    if (ptr_array == NULL || n <= 0) return -1;   
    for (int i = 0; i < n; i++) {
...
double max_abs(double* ptr_array, int n, double A) {
    if (ptr_array == NULL || n <= 0) return -1.0;
    double max_abs_val = -1.0;
    int found = 0;
    for (int i = 0; i < n; i++) {
        if (ptr_array[i] != A) {
            double abs_val = fabs(ptr_array[i]);
            if (!found || abs_val > max_abs_val) {
                max_abs_val = abs_val;
                found = 1;
            }
        }
...
double max_min_diff(double* ptr_array, int begin, int end, int n) {
    if (ptr_array == NULL || n <= 0) return -1.0;
    if (begin < 0 || end >= n || begin > end) return -1.0;
...
int delete_negatives(double* ptr_arr, int size) {
    if (ptr_arr == NULL || size <= 0) return 0;   
    int write = 0;
    for (int i = 0; i < size; i++) {
        if (ptr_arr[i] >= 0) {
...
double* insert(double* ptr_arr, int* size) {
    if (ptr_arr == NULL || size == NULL || *size <= 0) return NULL;
    int pos_count = 0;
    for (int i = 0; i < *size; i++) {
    ...
    int write_pos = size_n - 1;
    for (int i = *size - 1; i >= 0; i--) {
        ptr_arr_n[write_pos--] = ptr_arr_n[i];
        if (ptr_arr_n[i] > 0.0) {
...
double* create_arr_d(double* arr_a, double* arr_b, double* arr_c, 
    ...
    *size_d = 0;
    if (arr_a == NULL || arr_b == NULL || arr_c == NULL) return NULL;
    int neg = 0;
